...
import { initialProjects, initialSprints, initialTasks, initialBugs } from '../../services/mockData';

export const PmDashboard = ({ onNavigate }) => {
  const sprintBurndown = [
    { day: 'Day 1', remaining: 24, target: 24 },
    { day: 'Day 3', remaining: 19, target: 19 },
    { day: 'Day 5', remaining: 14, target: 14 },
    { day: 'Day 7', remaining: 9, target: 10 },
    { day: 'Day 9', remaining: 4, target: 5 },
    { day: 'Day 11', remaining: 1, target: 0 }
  ];

  const milestoneProgress = [
    { name: 'OAuth2 Hardening', completed: 90, total: 100 },
    { name: 'RBAC Frontend Integration', completed: 75, total: 100 },
    { name: 'SonarQube Quality Gate', completed: 30, total: 100 },
  ];

  const bugTrends = [
    { week: 'W1', open: 12, resolved: 8 },
    { week: 'W2', open: 8, resolved: 14 },
    { week: 'W3', open: 5, resolved: 11 },
    { week: 'W4', open: 2, resolved: 9 },
  ];

  const widgets = [
    { label: 'Projects', value: '3 Active', sub: '2 Delivery, 1 Planning', icon: FolderKanban, color: 'border-l-blue-500' },
    { label: 'Active Sprints', value: '2 Running', sub: 'Sprint 24 & Sprint 12', icon: Layers, color: 'border-l-indigo-500' },
    { label: 'Completed Tasks', value: '30 Tasks', sub: '78% of sprint scope', icon: CheckSquare, color: 'border-l-emerald-500' },
    { label: 'Pending Tasks', value: '12 Tasks', sub: 'In progress or review', icon: Clock, color: 'border-l-amber-500' },
    { label: 'Upcoming Deadlines', value: 'Sep 15', icon: Calendar, sub: 'Aureon Core Release', color: 'border-l-rose-500' },
    { label: 'Project Progress', value: '78%', icon: TrendingUp, sub: 'Overall completion', color: 'border-l-purple-500' },
    { label: 'Sprint Progress', value: '82%', icon: Activity, sub: 'On track for delivery', color: 'border-l-cyan-500' },
    { label: 'Project Health Score', value: '94 / 100', icon: Cpu, sub: 'Excellent health', color: 'border-l-emerald-600' },
    { label: 'Repository Status', value: 'Synced', icon: GitBranch, sub: '4 repositories connected', color: 'border-l-blue-600' },
    { label: 'Open Bugs', value: '2 Open', icon: Bug, sub: '1 Critical, 1 Low', color: 'border-l-rose-600' },
    { label: 'Team Workload', value: '84% Capacity', icon: Users, sub: 'Balanced distribution', color: 'border-l-slate-500' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-blue-500/20 text-blue-300 font-mono text-[10px] uppercase font-bold border border-blue-500/30">
              Project Delivery & Planning Hub
            </span>
          </div>
          <h1 className="text-2xl font-black tracking-tight mt-1">Software Delivery Center</h1>
          <p className="text-xs text-gray-300 mt-1 max-w-xl">
            Monitor active sprint burndowns, team workloads, project health metrics, and milestone timelines.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Layers className="w-3.5 h-3.5" /> Manage Sprints
          </button>
        </div>
      </div>
